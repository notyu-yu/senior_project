...
// Template payment token contract
...
// Template SGX verifier contract
...
contract GenomeBounty {
    // String literals of verifier files
    ...
    mapping(address => bool) public user_registered; // Map for verified users
    mapping(address => bool) public code_received; // Mark which sourcecodes were received
    mapping(address => uint256) public key_hash_map; // Stores submission key hashes
    mapping(address => uint256) public code_hash_map; // Stores submission code hashes
    mapping(address => bytes) public submitter_key_map; // Stores submission keys
    ...
    function rewardHandler(bytes calldata submitter_key) public validUser {
        require(
